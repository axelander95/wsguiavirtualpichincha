<?php
$str = get_vc_version();
$vc_version = preg_replace('/(.+?).(.+?)./', '$1.$2', $str);

if ( $vc_version >= '4.6' ){


$output = $el_class = $bg_image = $bg_color = $bg_image_repeat = $font_color = $padding = $margin_bottom = $css = $after_output = '';
...
$output .= 'class="'.esc_attr( trim( $css_class ) ).' '.$type.' ';
